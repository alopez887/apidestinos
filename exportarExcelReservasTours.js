// exportarExcelReservasTours.js
import ExcelJS from 'exceljs';
import pool from './conexion.js';
import path from 'path';

const exportarExcelReservasTours = async (req, res) => {
  try {
    ...
    // 🔹 SOLO TOURS, filtrando por fecha de REGISTRO (columna "fecha" ::date)
    const query = `
      SELECT 
        folio,
        nombre_cliente,
        ...
        fecha::date                     AS fecha_registro
      FROM reservaciones
      WHERE
        (UPPER(tipo_servicio) = 'TOURS' OR UPPER(tipo_viaje) = 'TOURS')
        AND fecha::date BETWEEN $1 AND $2
      ORDER BY fecha_registro ASC, folio ASC
    `;
    const { rows } = await pool.query(query, [desde, hasta]);

    const wb = new ExcelJS.Workbook();
    const ws = wb.addWorksheet('Reservas (Tours)');
    ...
    titleCell.value = 'REPORTE DE RESERVAS (TOURS) - CABO TRAVELS SOLUTIONS';
    titleCell.font = { bold: true, size: 16 };
    titleCell.alignment = { horizontal: 'center', vertical: 'middle' };

    // Encabezados (igual estilo + agrego "Fecha registro" al inicio)
    const headers = [
    ...
    rows.forEach((r, idx) => {
      ...
      headers.forEach((h, i) => {
        ...
        if (['aerolinea_salida', 'vuelo_salida', 'nombre_tour'].includes(h.key)) {
        ...
        if ((h.key === 'fecha_salida' || h.key === 'fecha_registro') && val instanceof Date) {
    ...
    res.setHeader('Content-Disposition', `attachment; filename="reservas_tours_${desde}_a_${hasta}.xlsx"`);
    res.setHeader('Content-Length', buffer.length);
    return res.status(200).end(Buffer.from(buffer));
  } catch (err) {
    console.error('❌ Error al generar Excel de reservas (Tours):', err);
    return res.status(500).json({ error: 'Error al generar Excel (Reservas Tours)' });
  }
};

export default exportarExcelReservasTours;
