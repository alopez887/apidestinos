// exportarExcelReservasAmbos.js
import ExcelJS from 'exceljs';
import pool from './conexion.js';
import path from 'path';

export default async function exportarExcelReservasAmbos(req, res) {
  try {
    ...
    // 🔹 Mismo concepto que exportarExcelSalidasAmbos,
    // pero ahora FILTRAMOS por la fecha de la RESERVA: columna "fecha" (::date)
    const sql = `
      WITH t_transporte AS (
        ...
        WHERE (
          ...
          AND (fecha::date BETWEEN $1 AND $2)   -- 👈 aquí usamos FECHA (reservación)
      ),
      t_tours AS (
        SELECT
          folio,
        ...
        WHERE tipo_servicio ILIKE 'tours'
          AND (fecha::date BETWEEN $1 AND $2)   -- 👈 también por FECHA (reservación)
      )
    ...
    const ws = wb.addWorksheet('Reservas (Ambos)');

    // Logo (opcional, igual que antes)
    try {
    ...
    titleCell.value = 'REPORTE DE RESERVAS (TRANSPORTE + TOURS)';
    titleCell.font = { bold: true, size: 16 };
    titleCell.alignment = { horizontal: 'center', vertical: 'middle' };

    // Encabezados unificados (los mismos que tu reporte de salidas)
    const headers = [
    ...
    // Enviar Excel por buffer (como en otros reportes nuevos)
    const buffer = await wb.xlsx.writeBuffer();
    res.setHeader('Content-Type','application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    res.setHeader('Content-Disposition', `attachment; filename="reservas_ambos_${desde}_a_${hasta}.xlsx"`);
    res.setHeader('Content-Length', buffer.length);
    return res.status(200).end(Buffer.from(buffer));
  } catch (err) {
    console.error('❌ Error al generar Excel de reservas (Ambos):', err);
    return res.status(500).json({ error: 'Error al generar Excel de reservas (Ambos)' });
  }
}
